...
func TestNewWithReadRecorder(t *testing.T) {
	...
	e, err := engine.EngineWithReadRecs(ctx, log, rec, m)
	if err != nil {
...
func TestEngineMultiReader(t *testing.T) {
	...
	e, err := engine.EngineWithReadRecs(ctx, log, rec, reds)
	if err != nil {
		t.Errorf("err = (%v); want (nil)", err)
	}
	done := make(chan struct{})
...
func TestEngineOnlyErrorsIfNoneOfReadersPinged(t *testing.T) {
	...
	e, err = engine.EngineWithReadRecs(ctx, log, rec, map[string]reader.DataReader{
		red1.Name(): red1,
		red2.Name(): red2,
	})
	if err == nil {
