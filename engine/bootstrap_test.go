...
func requirements(t *testing.T) (context.Context, *tools.Logger, *rdt.Reader, *rct.Recorder) {
...
func TestEmptyConfmapErrors(t *testing.T) {
	...
	d, err := engine.StartEngines(ctx, log, nil)
	if err == nil {
...
func TestEmptyReaderErrors(t *testing.T) {
	...
	d, err := engine.StartEngines(ctx, log, confMap)
	if err == nil {
		t.Error("err = (nil); want (error)")
	}
	if d != nil {
