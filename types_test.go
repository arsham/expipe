...
func TestGetQueryString(t *testing.T) {
    ...
    testCase := []struct {
        ...
        {
            []expvastic.DataType{&expvastic.FloatType{"test", 3.4}},
            fmt.Sprintf(`{%s,"test":%f}`, tStr, 3.4),
        },
        {
            []expvastic.DataType{&expvastic.StringType{"test", "3a"}, &expvastic.FloatType{"test2", 2.2}},
    ...
    for i, tc := range testCase {
        ...
        t.Run(name, func(t *testing.T) {
            contaner := expvastic.NewContainer(tc.input)
